...
def RK3evolve(y,CFL,nstep,rho0,s,RKopt, param=0.0,param0 =0.0,verb = False,time0=None, ru00=[], rv00=[], t00=[]):
	...
	#------------Physics -------------------------#
	ire = 1.0/160.0 #inverse of Reynolds
	ipe = 1.0/(160.0*0.7) #inverse of Peclet
        my = len(y)
        L  = y[-1]
	#---------------------------------------------#
	...
	#eigenvalue should be proportional to (mu/rho)^-1 instead of mu^-1
	Dtvisc = min(1.0/T00)*0.5*min(diff(y))**2/ire
	#Dtvisc = 0.5*min(diff(y))**3/ire/sratio**0.5
	#Dtvisc = 0.25*min(diff(y))**3/ire/sratio
	#Start istep 
	if RKopt==2: #Update using N(rho) directly (NOT low-storage)
		for istep in range(nstep):
		#Obtain timestep
			Dt = max(1.0/Dtvisc,max(rhov00*T00)/min(diff(y)))
			Dt = CFL/Dt
		...
		for istep in range(nstep):
		#Obtain timestep
			Dt = max(1.0/Dtvisc,2.0*max(rhov00*T00)/min(diff(y)))
