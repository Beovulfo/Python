...
def planeyj2vtk(filename,varpln,binary,pert,statfile,plyx=48,plyz=1,scale=1.0):
 	""" 
	...
	y = RECORD2['yfmap']['y'][0,]/scale
        jspecy = RECORD2['jspecy'][0,]

	Lx = 2*3.1415/RECORD1['alp']
	Lz = 2*3.1415/RECORD1['bet']
	x = np.linspace(-Lx/2.,Lx/2.,mgalx)/scale
	z = np.linspace(-Lz/2.,Lz/2.,mgalz)/scale
	...
	#ReadSTATS
	if pert>0.0:
		stats = readstat(statfile)
		print 'file %s loaded' %(statfile)
		yy=stats['y']/scale
		fmeany=stats['mean']
		fmax = np.max(stats['mean'])
		fmin = np.min(stats['mean'])
	...
	if varpln[0:2]=='Tf': #Scale by DT and take the minimum (set from 0 to 1)
		...
		if pert>0.0:
			for j in range(my):
				...
				FIELD1[:,j] = np.multiply(np.subtract(rectdata[0,:],fmin),fact)
		else:
			for j in range(my):
    				readrec = np.fromfile(f,rect,1)
    				rectdata = readrec['data']
...
def planexz2vtk(filename,varpln,binary,pert,pln1,pln2,statfile,scale=1.0):
	import scipy as sc
	import numpy as np
	import pylab
	from readfiles import readstat
#	import fortran2vtk
	f = open(filename,'rb')
        planeij = filename[-2:]
	#Create dtypes for proper reading from Fortran unformatted
	...
	z = np.linspace(-Lz/2.,Lz/2.,mgalz)/scale
	y = y/scale
	print 'Reading VOLUME XZ and scaling coordinates by %s \n' % scale
	...
	for j in range(nspec):
		...
		if j< pln1 or j>pln2:
			continue
		print "plane at y/scale = %s" % y[jspecy[j]-1]
		pos=y[jspecy[j]-1]
		sig = 'p'
		if pos<0:
			sig='m'
		fnpln=filename+'y'+sig+ '%5.5i'% (abs(pos*100))
		# If scale different to one, change the name of the file to reflect this
		if scale != 1.0:
			fnpln=filename+'ydel'+sig+ '%5.5i'% (abs(pos*100))
		#T is special case
		if (varpln[0:2]=='Tf' and pert>0.0):
			...
			fact = 1.0
			f0=0.0
		planeydata=np.multiply(np.subtract(planeydata,f0),fact)
		planeydata.shape=(mgalz,mgalx)
