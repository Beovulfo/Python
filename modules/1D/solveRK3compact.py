...
def RK3evolve(y,CFL,nstep,rho0,s,RKopt, param=0.0,param0 =0.0,verb = False,time0=None, ru00=[], rv00=[], t00=[]):
	...
	ipe = 1./(Reynolds*0.7) #inverse of Peclet
        my = len(y)
        L  = y[-1]
	#---------------------------------------------#
	...
	rhocrit = min(rhobot,rhotop)

	Dtvisc =Reynolds/2.0*rhocrit*min(diff(y))**2
	#Start istep 
		...
		#		Dt = 0.01*Dtvisc #first step is important
			...
			#For each rkstep
				...
				if rkstep == 0: #First rkstep
					...
					drhodt = -DT00/(beta[rkstep]*Dt*T00last*T00)-alpha[rkstep]/beta[rkstep]*drhodt
				M = trapz(drhodt,y)
				rhov00 = -cumtrapz(drhodt,y,initial=0.0)+M*(1.0/(1+(rhotop/rhobot)**0.5)
				#rhov00 = -cumtrapz(drhodt,y,initial=0.0)+M*rhobot/(rhobot+rhotop)
			time = time + Dt
			dm[istep]=calcdm(y,rhou00*T00)
			timev[istep]=time
			if isNaN(dm[istep]):
